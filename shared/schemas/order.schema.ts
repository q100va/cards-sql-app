...
import {
  OCCASION_TYPES,
  STATUSES as OCCASION_STATUSES,
} from '../constants/occasions.js';
...
const occasionTypeSchema = positiveInt.refine(
  (value) =>
    OCCASION_TYPES.some(
      (occasionType) => occasionType.id === value,
    ),
);

const occasionStatusSchema = positiveInt.refine(
  (value) =>
    OCCASION_STATUSES.some(
...
export const filterSchema = z
  .object({
    addressCategory:
      addressCategoryFilterSchema.optional(),

    gender:
      genderFilterSchema.optional(),
  ...
  .superRefine((filters, ctx) => {
    if (
      ...
      ctx.addIssue({
        ...
        path: ['date2'],
        message:
          'FORM_VALIDATION.ORDER.INVALID_DATE_RANGE',
      });
    }
    ...
    if (
      ...
      ctx.addIssue({
        ...
        path: ['year2'],
        message:
          'FORM_VALIDATION.ORDER.INVALID_YEAR_RANGE',
      });
    }

    if (
      filters.minFromOneHouse != null &&
      ...
      ctx.addIssue({
        ...
        message:
          'FORM_VALIDATION.ORDER.MIN_GREATER_THAN_MAX',
      });
    }
    ...
    if (
      ...
      ctx.addIssue({
        ...
        path: ['maxNoAddress'],
        message:
          'FORM_VALIDATION.ORDER.MAX_NO_ADDRESS_NOT_ALLOWED',
      });
    }
    ...
    if (
      ...
      ctx.addIssue({
        ...
        path: ['gender'],
        message:
          'FORM_VALIDATION.GENDER_PROPORTION_NOT_ALLOWED',
      });
    }
    ...
    if (
      ...
      ctx.addIssue({
        ...
        path: ['gender'],
        message:
          'FORM_VALIDATION.GENDER_AMOUNTS_NOT_ALLOWED',
      });
    }
  })
  .optional();
    ...
    ({ orderDraft, filters }, ctx) => {
      ...
      if (
        ...
        ctx.addIssue({
          ...
          message:
            'FORM_VALIDATION.ORDER.GENDER_AMOUNT_SUM',
        });
      }

      if (
        filters.maxNoAddress != null &&
        ...
        ctx.addIssue({
          ...
          message:
            'FORM_VALIDATION.ORDER.MAX_NO_ADDRESS_TOO_BIG',
        });
      }
      ...
      if (
        ...
        ctx.addIssue({
          ...
          message:
            'FORM_VALIDATION.ORDER.MIN_FROM_ONE_HOUSE_TOO_BIG',
        });
      }
      ...
      if (
        ...
        ctx.addIssue({
          ...
          message:
            'FORM_VALIDATION.ORDER.MAX_FROM_ONE_HOUSE_TOO_BIG',
        });
      }
      ...
      if (
        ...
        ctx.addIssue({
          ...
          message:
            'FORM_VALIDATION.ORDER.IMPOSSIBLE_HOUSE_DISTRIBUTION',
        });
      }
    },
  );
...
export const orderFilterRegionsAndHomesSchema =
  z
    .object({
      regions: z.array(
        z
...
export type OccasionNodeData = z.infer<
  typeof occasionNodeDataSchema
>;
...
export const occasionNodeSchema:
  z.ZodType<OccasionNode> = z.lazy(() =>
    z
      .object({
        key: z.string(),
        label: z.string(),
...
export const occasionNodesSchema =
  z.array(occasionNodeSchema);
...
export type OrderFilterRegionsAndHomes =
  z.infer<
    typeof orderFilterRegionsAndHomesSchema
  >;

export type OrderDraft =
  z.infer<typeof orderDraftSchema>;
...
export type OrderRecipients =
  z.infer<typeof orderRecipientsSchema>;

export type OrderRecipient =
  z.infer<typeof orderRecipientSchema>;

export type OrderQuery =
  z.infer<typeof orderQueryDTOSchema>;
