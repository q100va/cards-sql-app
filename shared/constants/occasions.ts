...
export const YEARS = Array.from(
  { length: LAST_OCCASION_YEAR - FIRST_OCCASION_YEAR + 1 },
  (_, index) => FIRST_OCCASION_YEAR + index,
);

export const OCCASION_TYPES_BY_ID = Object.fromEntries(
  OCCASION_TYPES.map((item) => [item.id, item]),
);

export const MONTHS_BY_ID = Object.fromEntries(
  MONTHS.map((item) => [item.id, item]),
);

export const STATUSES_BY_ID = Object.fromEntries(
  STATUSES.map((item) => [item.id, item]),
);
